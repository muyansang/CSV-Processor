...
class CsvJoinTest {
    ...
    void testJoin() throws IOException {
        ...
        testJoinHelper("student");
        testJoinHelper("example");

        // WHEN there are duplicate left keys and there is at most one match per key
        testJoinHelper("student2");
        testJoinHelper("states");

        // WHEN the left keys are unique and there is more than one match per key
        testJoinHelper("student3");

        //  WHEN there are duplicate left keys and there is more than one match per key
        testJoinHelper("student4");

        //  WHEN there are empty fields
        testJoinHelper("student5");
    }
